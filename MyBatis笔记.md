...
什么是分页：如果一次性查询全部数据，有可能导致响应时间过长，用户体验较差。分页可以解决该问题，一次只显示一部分数据。



实现分页：要制定好每页多少条数据pageSize、当前的页数pageNum，由此可以得知当前页的起始索引index：

`index=（pageNum-1）*pageSize`

实现分页可以有多种方式，例如：

1. sql语句中使用limit index,pageSize，会查询从index行开始的后续pageSize条数据，通过前端传参pageNum即可查询并返回数据。
2. 使用MyBatis自带的分页功能，需要使用PageHelper插件，在核心配置文件中配置<plugin>，即可使用PageHelper对查询结果进行分页，功能强大，较为推荐。
   - 做法是，使用PageHelper.startPage(int pageNum,int pageSize)开启分页
   - 使用PageInfo获取分页相关信息
